...
angular.module('starter', ['ionic', 'ui.gravatar'])
...
.controller('HomeCtrl', function ($scope, $rootScope) {
  ...
  var musicDir = path.join(process.env['USERPROFILE'], 'Music');

  var readdir = function () {
    fs.readdir(musicDir, function (err, list) {
      if (err) throw err;
      var music = [];
      list.forEach(function (file) {
        if (/.*\.(mp3|ogg|oga|wav|wma)$/.test(file)) {
          music.push(file);
        }
      });
      $scope.library = music;
...
.controller('SettingsCtrl', function ($scope, $rootScope, $ionicLoading) {
  ...
  $scope.openDevTools = function () {
    var r = $rootScope.getCurrentWindow().openDevTools();
    console.log(r);
  }
